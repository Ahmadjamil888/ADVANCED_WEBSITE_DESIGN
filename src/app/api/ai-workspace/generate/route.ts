...
async function uploadFileToSpace(spaceName: string, fileName: string, content: string, hfToken: string) {
  ...
  try {
    // Use the correct HuggingFace Hub API endpoint for file uploads
    ...
    if (response.ok) {
      ...
      // Try with different API endpoint format
      console.log(`🔄 Trying alternative API endpoint for ${fileName}...`);
      const altUrl = `https://huggingface.co/api/repos/Ahmadjamil888/${spaceName}/upload/main/${fileName}`;
      ...
      if (altResponse.ok) {
        console.log(`✅ ${fileName} uploaded successfully (alternative endpoint)`);
        return true;
      } else {
        const altError = await altResponse.text();
        console.error(`❌ Alternative upload failed for ${fileName}:`, altResponse.status, altError);
        
        // Try with PUT method (some APIs prefer PUT for file uploads)
        console.log(`🔄 Trying PUT method for ${fileName}...`);
        const putResponse = await fetch(altUrl, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${hfToken}`,
            'Content-Type': 'application/json'
        ...
        if (putResponse.ok) {
          console.log(`✅ ${fileName} uploaded successfully (PUT method)`);
          return true;
        } else {
          const putError = await putResponse.text();
          console.error(`❌ PUT upload failed for ${fileName}:`, putResponse.status, putError);
          return false;
        }
      }
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const [fileName, content] of Object.entries(generatedFiles)) {
      ...
      const result = await uploadFileToSpace(spaceName, fileName, content as string, hfToken);
